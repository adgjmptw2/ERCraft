...
describe('analysisUiLabels', () => {
  ...
  it('confidence 라벨', () => {
    expect(getConfidenceLabel('high')).toBe('분석 신뢰도 높음')
    expect(getConfidenceLabel('insufficient')).toBe('표본 부족')
  })

  it('status badge 라벨', () => {
    expect(getStatusBadgeLabel('ready')).toBe('계산됨')
    expect(getStatusBadgeLabel('future', 'requiresMatchDetail')).toBe('상세 데이터 필요')
