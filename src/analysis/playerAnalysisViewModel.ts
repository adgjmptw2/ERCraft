...
const AXIS_KEYWORDS: Record<AnalysisAxis, string> = {
  survival: '생존 안정',
  combat: '교전 영향',
  macro: '운영 효율',
  support: '팀 기여',
  ...
  switch (id) {
    ...
    case 'roleFitScore': {
      ...
      const top = Object.entries(playStyle.roleFitScores)
        .filter((entry): entry is [PlayerRole, number] => entry[1] != null)
        .sort((a, b) => b[1] - a[1])[0]
      return top?.[1] ?? null
    }
    default:
...
function buildHeadline(playStyle: PlayerPlayStyleAnalysis | null): string {
  if (!playStyle || playStyle.status !== 'ok') return '표본 부족'
  const topAxis = ANALYSIS_AXES.map((axis) => ({
    axis,
    score: playStyle.axisScores[axis] ?? 0,
  }))
    .sort((a, b) => b.score - a.score)[0]
  if (!topAxis || topAxis.score <= 0) return '성향 분석 준비 중'
  return `${ANALYSIS_AXIS_LABELS[topAxis.axis]} 중심 플레이`
}
...
function buildInsightLine(
  ...
  if (playStyle?.status === 'ok' && playStyle.comment) return playStyle.comment
  if (report?.summary) return report.summary
  return '최근 경기 기준 룰 기반 요약입니다.'
}

function resolveDataConfidence(sampleSize: number): AnalysisDataConfidence {
...
export function buildPlayerAnalysisViewModel(
  ...
  const { playStyleAnalysis, analysisReport, analysisMatches, basisLabel } = params

  const sampleSize = playStyleAnalysis?.sampleSize ?? analysisMatches.length
  ...
  const radarAxes =
    playStyle != null
      ? ANALYSIS_AXES.flatMap((axis) => {
          const score = playStyle.axisScores[axis]
          if (score == null) return []
          return [
            {
              axis,
              label: ANALYSIS_AXIS_LABELS[axis],
              score,
              keyword: AXIS_KEYWORDS[axis],
            },
          ]
        })
      : []
  ...
  const estimatedTendency = playStyle?.primaryRole
    ? `추정: ${roleFitLabel(playStyle.primaryRole)}`
    : null
  const secondaryTendency = playStyle?.secondaryRole
    ? `보조: ${roleFitLabel(playStyle.secondaryRole)}`
    : null

  return {
    sampleSize,
    sampleLabel: basisLabel,
    headline: buildHeadline(playStyle),
    insightLine: buildInsightLine(playStyle, analysisReport),
    dataConfidence: resolveDataConfidence(sampleSize),
    estimatedTendency,
    secondaryTendency,
    summaryMetrics,
